...
func buildPage() Node {
	return HTML(
		...
		Body(
			...
			// Bind Modifiers Demo
			Div(Class("demo-section"),
				H2(Text("Bind Modifiers")),
				P(Text("Passing modifiers to data-bind emits the key form of the attribute, so Datastar reads the signal name from the attribute key instead of its value.")),
				Div(Class("grid"),
					Div(Class("demo-box"),
						H3(Text("Bind to a Property")),
						P(Text("data-bind:accepted__prop.checked binds the signal to the checked property.")),
						Label(
							Input(Type("checkbox"), data.Bind("accepted", data.Prop("checked"))),
							Text(" I accept"),
						),
						P(Text("Accepted: "), Span(data.Text("$accepted"))),
						Button(data.On("click", "$accepted = !$accepted"), Text("Toggle from a signal")),
					),
					Div(Class("demo-box"),
						H3(Text("Choose the Sync Event")),
						P(Text("data-bind:search__event.change syncs on change only, so the signal updates when the input loses focus or you press Enter, not on every keystroke.")),
						Input(Type("text"), data.Bind("search", data.Event("change")), Placeholder("Type, then press Enter")),
						P(Text("Search: "), Span(data.Text("$search"))),
					),
				),
			),
